...
class table_column_tool_TableColumnTool {
  ...
  updateToolCells() {
    const tableContainer = external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default.a.find(this.table);
		var iterator = tableContainer.children.head;
		while (iterator.statics.tagName != 'TBODY' && iterator != null) {iterator = iterator.next;}
		const CellsInFirstRow = iterator.children.head.children;
...
const List = external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default.a.import("formats/list");
    ...
    if (CELL_ATTRIBUTES.concat(CELL_IDENTITY_KEYS).indexOf(name) > -1) {
      ...
      super.format(name, {
        ...
        colspan
			...
			console.log(this.domNode.firstChild);
			if (this.domNode.firstChild.tagName == 'OL') {
				// if (this.domNode.firstChild.childNodes.length < 2) {
				// 	var item_node = this.domNode.firstChild.firstChild.removeAttribute('data-list');
				// 	this.domNode.firstChild.remove();
				// 	this.domNode.appendChild(item_node);
				// }
				// console.log(this.domNode.firstChild.remove();
				// this.domNode.firstChild.firstChild.removeAttribute('data-list');
			} else {
				if (this.domNode.firstChild.tagName == 'BR') {
					this.domNode.firstChild.remove();
					var item_child = document.createElement('br');
				} else {
					var item_child = this.domNode.firstChild;
				}
				let list_node = document.createElement('ol');
				let item_node = document.createElement('li');
				item_node.setAttribute('data-list', value);
				item_node.appendChild(item_child);
				list_node.appendChild(item_node);
				this.domNode.appendChild(list_node);
			}
			// this.appendChild(tableCell_list);
			// this.children.forEach(child => {
			// 	if (child.statics.tagName == 'BR') {
			// 		var replace_node = this.scroll.create('DIV');
			// 	} else {
			// 		console.log(child);
			// 		var replace_node = child;
			// 	}
			// 	replace_node.format(name, value);
			// });
		  ...
		  // }
			...
			// quill.setSelection(1, external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default.a.sources.USER);
    } else {
			//bookmark . ted
			if (value != null) {
				this.domNode.parentNode.style.backgroundColor = value['cell-bg'];
			}
      super.format(name, value);
    }
  }
...
class table_TableContainer extends Container {
  ...
  colGroup() {
		if (this.children.head != null) {
			if (this.children.head.statics.tagName == 'COLGROUP') {
...
quill_better_table_BetterTable.keyboardBindings = {
  ...
  'up-to-table': {
    ...
    handler(range, context) {
			...
			if (context.line.children.head.statics.tagName == 'TABLE' && context.line.next.children.head.statics.tagName == 'TABLE') {
				var prev_table = context.line.prev.children.head;
				var next_table = context.line.next.children.head;
				//
				var iterator = prev_table.children.head;
				while (iterator.statics.tagName != 'COLGROUP' && iterator != null) {iterator = iterator.next;}
				var prev_table_colnum = iterator.children.length;
				//
				var iterator = next_table.children.head;
				while (iterator.statics.tagName != 'COLGROUP' && iterator != null) {iterator = iterator.next;}
				var next_table_colnum = iterator.children.length;
				//
				if (prev_table_colnum == next_table_colnum) {
					return true;
				} else {
					return false;
				}
				...
				// 	while (iterator.statics.tagName != 'TBODY' && iterator != null) {iterator = iterator.next;}
				// 	var rows = [];
				// 	var iterator = iterator.children.head;
				// 	while (iterator != null) {
				// 		rows.push(iterator);
				...
				// 	while (iterator.statics.tagName != 'TBODY' && iterator != null) {iterator = iterator.next;}
				// 	for (var row in rows) {
				// 		iterator.appendChild(rows[row]);
				// 	}
				// 	context.line.next.remove();
				//
				// 	return false;
				// } else if (prev_table_colnum < next_table_colnum) {
				// 	var col_diff = next_table_colnum - prev_table_colnum;
				// 	var iterator = next_table.children.head;
				// 	while (iterator.statics.tagName != 'TBODY' && iterator != null) {iterator = iterator.next;}
				// 	var rows = [];
				// 	var iterator = iterator.children.head;
				...
				// 	var iterator = iterator.children.head;
				// 	while (iterator != null) {
				// 		iterator.children.tail.format('colspan', parseInt(iterator.children.tail.formats().colspan) + col_diff);
				// 		iterator = iterator.next;
				// 	}
				// 	var iterator = prev_table.children.head;
				// 	while (iterator.statics.tagName != 'TBODY' && iterator != null) {iterator = iterator.next;}
				...
				// 	return false;
				// } else {
				// 	var col_diff = prev_table_colnum - next_table_colnum;
				// 	var iterator = next_table.children.head;
				// 	while (iterator.statics.tagName != 'TBODY' && iterator != null) {iterator = iterator.next;}
				// 	var rows = [];
				// 	var iterator = iterator.children.head;
    ...
    handler(range, context) {
			console.log(context.line);
			if (context.line.prev.children.head.statics.tagName == 'TABLE' && context.line.next.children.head.statics.tagName == 'TABLE') {
				console.log('in');
				var prev_table = context.line.prev.children.head;
				var next_table = context.line.next.children.head;
				//
				...
				// 	return false;
				// }
			} else if (context.line.next.children.head.statics.tagName == 'TABLE') {
				return false;
			}
		}
  },
    ...
    // format: ['table-cell-line'],
    handler(range, context) {
			if (context.line.next.children.head.statics.tagName == 'TABLE') {
				return false;
			}
		}
  },
