...
quill_better_table_BetterTable.keyboardBindings = {
  ...
  'up-to-table': {
	...
	'delete-combine-table': {
    key: 'Delete',
    handler(range, context) {
			if (context.line.prev.statics.blotName == 'table-view' && context.line.next.statics.blotName == 'table-view') {
				var iterator = context.line.prev.children.head.children.head;
				while (iterator.statics.tagName != 'COLGROUP' && iterator != null) {iterator = iterator.next;}
				var prev_colnum = iterator.children.length;
				var iterator = context.line.next.children.head.children.head;
				while (iterator.statics.tagName != 'COLGROUP' && iterator != null) {iterator = iterator.next;}
				var next_colnum = iterator.children.length;

				if (prev_colnum == next_colnum) {
					return true;
				} else {
					return false;
				}
